...
/**
 * This is a class that defines the environment of a Tic Tac Toe Game: its underlying state representation, actions transitions etc. 
 * It also provides functionality for playing out a game between two agents, various other useful methods.
 * 
 * 
 * 
 *  
...
public class Game {
	...
	/**
	 * this integer represents the state of the game, and is set by {@link #evaluateGameState()} after every move
	 * DO NOT CONFUSE THIS with an MDP state.
	 */
	int state=0;
	...
	/**
	 * 
	 * @return All possible next game states from the current game state in one step where the agent whose
	 * turn it is moves.
	 * 
	 */
	...
	/**
	 * A static method to generate all valid game states where it is either X's turn or O's turn as specified by {@code xo} argument.
	 * NOTE: this method will include ALL terminal states in the game.
	...
	{
		List<Game> result=new ArrayList<Game>();
		//this is the number of possible game hashes (2222222222 in base 3): 59048 - not all are valid. Here we only return the valid ones.
...
//				System.out.println("--------");
	...
	/**
	 * returns a list of all available moves by the agent whose turn it is NOT. Used by the defensive agent.
	 * @return
	 */
	...
	/**
	 * Evaluates the game, and sets its state to one of DRAW, ONGOING, X_WIN or O_WIN
	...
	/** Executes move on the current game. Returns the resulting game without changing the current one.
	 * 
	...
	/**
	 * Executes the move m on the current game. Returns the resulting game without changing the current one.
	 * 
	 * @param m
	...
	/** 
	 *  This method will play out the game to the end using moves from the X and O agents.
	 *  Use this method to test your agents
	 *  
	...
	/**
	 * A hashCode function implemented assuming the game board is a 10 digit number in base three, where:
	 * ' ' maps to 0
	 * 'X' maps to 1
	 * 'O' maps to 2
	 * 
	 * 10 digits instead of 9 because we used the last, 10th one to encode whose turn it is.
	 * 
	 * The returned value is essentially that number converted to base 10.
	 * 
	 * You can use this method to store (e.g. write to file) your value functions or policies.
	 * 
	...
	/**
	 * Given a hash value for a game according to {@link Game#hashCode} this method turns it back into a Game object,
	 * i.e. it is the inverse of the hashCode method.
	 * 
	...
	public static Game inverseHash(int hash)
	{
		...
		for(int l=s.length()-2;l>=0;l--)
		{
			...
			int j=(int)l%3;
			switch (s.charAt(l)) {
			case '0': g.board[i][j]=' '; break;
			case '1': g.board[i][j]='X'; break; 
			case '2': g.board[i][j]='O'; break;
...
//		g.executeMove('O', 1, 1);
//		
//		List<TransitionProb> transitions=mdp.generateTransitions(g, new Move('X', 0, 1));
...
//	}
	 ...
	 * 
	 * @param a
	 * @throws throws IllegalMoveException of the there is an illegal move by one of the agents. This would just exit the programme.
	 */
